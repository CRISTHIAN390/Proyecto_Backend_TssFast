...
def create_cliente(user: UserCreate):
    ...
    try:
        cursor.execute('''INSERT INTO clients (first_name, last_name, DNI, age, sex, username, email, password, role_id, date_created,state) 
                          VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, NOW(),%s)''',
                       (user.first_name, user.last_name, user.DNI, user.age, user.sex, user.username, user.email, hashed_password, user.role_id, user.state))
        conn.commit()
...
def delete_cliente(client_id: int):
    ...
    try:
        ...
        cursor.execute("UPDATE clients SET state = 0 WHERE id = %s", (client_id,))
        conn.commit()
    except mysql.connector.Error as err:
        conn.rollback()
        raise HTTPException(status_code=400, detail=str(err))
